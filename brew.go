...
// BrewInstall describes a detected Homebrew-managed installation.
...
// breaks `<tool> update` on non-brew hosts while a false negative only leaves
// us on the pre-existing self-update path.
...
func detectBrewInstall(ctx context.Context, env brewEnv) (*BrewInstall, error) {
	...
	if !confirmed && brewPath != "" {
		if reported, err := env.brewPrefix(ctx, brewPath); err == nil {
			// TrimSpace here as well as in execBrewPrefix: a prefix that
			// arrives with a stray newline would silently fail to confirm a
			// legitimate non-standard Homebrew install.
			confirmed = filepath.Clean(strings.TrimSpace(reported)) == cellar.prefix
		}
	}
	if !confirmed {
		return nil, nil
	...
	// conservative rule above is what keeps updates working on non-brew hosts.
...
// <prefix>/Cellar/<keg>/<version>/bin/<exe> into its parts. ok is false when
// the path has no "Cellar" component with a name after it.
func splitCellarPath(p string) (cellarPath, bool) {
	p = filepath.Clean(p)
...
func UpgradeViaBrew(ctx context.Context, inst *BrewInstall, binary string, stdout, stderr io.Writer) error {
	if inst == nil {
	...
	if inst.BrewPath == "" {
		// An *ErrManaged so a caller (and Main) treats this the same as any
		// other "you must not touch this install" outcome.
		return &ErrManaged{
			Path: inst.ExePath,
			Reason: fmt.Sprintf("%s is installed by Homebrew but `brew` is not on PATH; "+
				"self-updating would overwrite the keg and be reverted by the next `brew upgrade`", binary),
			Hint: fmt.Sprintf("put brew on PATH and re-run `%s update`", binary),
		}
	}
